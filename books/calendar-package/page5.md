...
これで、先述した通りに `Calendar(date: DateTime(2022, 6))` のようなインターフェースで呼び出せるようになりました。
...
`_WeekRow` に必要なのは「1 週間分の表示文字列リスト」です。例えば 6 日 ~ 12 日 の 1 週間分であれば `['6', '7', '8', '9', '10', '11', '12']` というリストになり、ヘッダ行であれば `['月', '火', '水', '木', '金', '土', '日']` です。
