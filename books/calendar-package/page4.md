...
つまり、引数で受け取った `date` オブジェクトの年と月は保持しつつ、日が 1 である新しい `DateTime` オブジェクトを生成し、そのオブジェクトの `.weekday` を呼び出せば良さそうですので、 `_calcFirstWeekday` メソッドは以下のように実装できます。
...
この 2 つの情報が計算できました。これらを使ってカレンダーを生成していきましょう。
